...
for s = 1:Ns
    ...
    Xtrn = reshape(permute(Xdata(:,:,Sdex),[1,3,2]),[Nt*Nss,Nx]);
    Ytrn = reshape(permute(Ydata(:  ,Sdex),[1,3,2]),[Nt*Nss,Ny]);
    ...
    if Mswitch(m)
        ...
        for ss = 1:Nss
            XXtrn{ss} = squeeze(Xdata(:,:,Sdex(ss)))';
            YYtrn{ss} = squeeze(Ydata(:  ,Sdex(ss)))';
        end
